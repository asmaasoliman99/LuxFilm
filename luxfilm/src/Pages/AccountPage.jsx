...
const AccountPage = () => {
  ...
  const updateSchema = z.object({
    fullName: z
      ...
      .refine((n) => n.trim().split(" ").length >= 2, {
        message: t("firstLastNameRequired"),
      }),
    userName: z
      .string()
      .min(3, t("usernameMin"))
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col items-center justify-center px-6 text-[var(--text-primary)]">
        ...
        <h2 className="text-3xl font-black mb-3">{t("accessDenied")}</h2>
        <p className="text-gray-400 mb-8 text-center max-w-sm">
          {t("signInToViewAccountDetails")}
        </p>
        <button
  ...
  const initials = (user.fullName || user.email || t("userFallback"))
    .split(" ")
    .slice(0, 2)
    .map((w) => w[0])
    .join("")
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      const found = users.find(
        (u) => u.id === user.id && u.password === validated.currentPassword,
      );
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pt-24 pb-20 px-4 md:px-12">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#842A3B]/20 flex items-center justify-center group-hover:bg-[#842A3B]/30 transition">
                    <Heart
                      size={18}
                      className="text-[#842A3B] fill-[#842A3B]"
                    />
                  </div>
                  <div>
                    <p className="text-2xl font-black">{wishlistCount}</p>
                    <p className="text-gray-500 text-xs">{t("myWishlist")}</p>
                  </div>
                </div>
              </Link>
              ...
              <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs">{t("memberSince")}</p>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold flex items-center gap-2">
                  <User size={18} className="text-[#842A3B]" />{" "}
                  {t("profileDetails")}
                </h2>
                {!editMode ? (
                  <button
                ...
                <form onSubmit={handleSave} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className={fieldClass("userName")}>
                      <span className="text-gray-500 text-sm font-bold flex-shrink-0">
                        @
                      </span>
                      <input
                        type="text"
            ...
            <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold flex items-center gap-2">
                  <Lock size={18} className="text-[#842A3B]" />{" "}
                  {t("changePassword")}
                </h2>
                {!pwMode ? (
                  <button
                    ...
                    <div key={key}>
                      ...
                      <div className={fieldClass(key)}>
                        ...
                        <input
                          ...
                          value={pwData[key]}
                          onChange={(e) =>
                            setPwData({ ...pwData, [key]: e.target.value })
                          }
                          className="bg-transparent border-none outline-none ml-3 w-full text-white text-sm placeholder:text-gray-600"
                          placeholder="••••••••"
                        />
          ...
          <div className="space-y-6">
            <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-base font-bold flex items-center gap-2">
                  <Heart size={16} className="text-[#842A3B] fill-[#842A3B]" />{" "}
                  {t("myWishlist")}
                </h2>
                <Link
                  to="/wishlist"
                  ...
                  {wishlist.slice(0, 4).map((movie) => (
                    ...
                    >
                      <img
                        ...
                        onError={(e) => {
                          e.target.src =
                            "https://via.placeholder.com/40x56/1c1c1c/666";
                        }}
                      />
                      <div className="flex-1 min-w-0">
            ...
            <div className="bg-[#1a1a1a] border border-red-900/20 rounded-2xl p-6">
              <h2 className="text-base font-bold text-red-400 mb-4">
                {t("dangerZone")}
              </h2>
              <button
                onClick={handleLogout}
              ...
              >
                <LogOut size={15} /> {t("signOutOfAccount")}
              </button>
            </div>
          </div>
        </div>
