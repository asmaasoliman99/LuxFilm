...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://api.themoviedb.org/3/genre/movie/list?api_key=${API_KEY}&language=${currentLang}`)
    ...
    <nav className={`fixed top-0 w-full z-50 flex items-center justify-between px-4 py-4 lg:px-12 transition-all duration-500 ${isScrolled ? 'bg-[#141414]' : 'bg-transparent bg-gradient-to-b from-black/70 to-transparent'}`}>
      ...
      <div className="flex items-center gap-6 lg:gap-10">
        ...
        <ul className="hidden md:flex items-center gap-5 text-sm text-[#e5e5e5]">
          <li className={navItemClass}><Link to="/">{t('navbar.home')}</Link></li>
          <li className={`relative group flex items-center gap-1 ${navItemClass}`}>
            {t('navbar.genres')} <ChevronDown size={14} />
            <div className="absolute top-full start-0 mt-2 w-48 bg-[#141414] border border-gray-700 rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[100]">
          ...
          <li className={navItemClass}><Link to="/wishlist">{t('navbar.myList')}</Link></li>
        </ul>
      </div>
      ...
      <div className="flex items-center gap-4 lg:gap-6 text-white">
        ...
        <div className={`flex items-center bg-black/60 border transition-all px-3 py-2 rounded-full overflow-hidden ${isSearchOpen ? 'w-44 lg:w-72 border-white/80' : 'w-16 border-white/40'}`}>
          ...
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} autoFocus={isSearchOpen} placeholder={t('navbar.searchPlaceholder')} className={`bg-transparent outline-none text-sm px-3 transition-all ${isSearchOpen ? 'w-full opacity-100 ms-2' : 'w-0 opacity-0 pointer-events-none'}`} />
        </div>
        ...
        <div className="relative">
          ...
          {isLangOpen && (
            <div className="absolute top-full end-0 mt-2 w-20 bg-[#141414] border border-white/10 rounded-md shadow-2xl py-1 z-[100] animate-in fade-in zoom-in duration-200">
              {['en', 'ar'].map((l) => (
                <button
                  key={l}
                  onClick={() => { setLang(l); setIsLangOpen(false); }}
                  className={`w-full text-start px-3 py-2 text-[10px] font-bold transition-colors uppercase hover:bg-[#842A3B] ${lang === l ? 'text-[#842A3B] hover:text-white' : 'text-gray-300'}`}
                >
                  {l}
                </button>
              ))}
        ...
        <Link to="/wishlist" className="relative">
          ...
          {wishlistCount > 0 && <span className="absolute -top-2 -end-2 min-w-[18px] h-[18px] bg-[#842A3B] text-[10px] rounded-full flex items-center justify-center shadow-lg">{wishlistCount > 99 ? '99+' : wishlistCount}</span>}
        </Link>
        ...
        {user ? (
          <div className="flex items-center gap-3">
            <Link to="/account" className="flex items-center gap-2 group">
              <span className="hidden lg:block text-xs font-semibold text-gray-300 group-hover:text-white transition">{t('navbar.hi')}{user.fullName?.split(' ')[0] || user.email?.split('@')[0]}</span>
          ...
          <div className="flex items-center gap-2 cursor-pointer hover:text-[#842A3B] transition" onClick={() => navigate('/login')}>
            <LogIn size={24} />
            <span className="text-sm font-bold hidden md:block">{t('navbar.login')}</span>
          </div>
        )}
      </div>
    </nav>
